...
def clickon(event):
    ...
    if event.inaxes is not None and cir==1:
        ...
        plt.draw()
def release(event):
    global pressed,x_data,y_data,cir,circle_number,line_number
    if event.inaxes is not None and cir==0:
        ax.scatter(event.xdata, event.ydata, c='m', s=10)
        pressed=0
        plt.draw()
        x_data = []
        y_data = []
    elif event.inaxes is not None and len(x_data)>=2 and cir==1: # x,y同步，因此只要判断一个
        x1=x_data[0]
        y1=y_data[0]
        x2=x_data[-1]
        y2=y_data[-1]
        ...
        if line_number>=1:
            ...
            fig.canvas.draw()
        # if circle_number>=1:
        #     for r in exist_radius[-circle_number:]:
        #         ax.add_artist(r)
        #     fig.canvas.draw()
        plt.draw()
        x_data = []
        ...
        plt.scatter(event.xdata,event.ydata,c='m')
def moving(event):
    ...
    if event.name == 'motion_notify_event' and pressed==1 and event.inaxes is not None and cir+lin==0:
        x_data.append(event.xdata)            # 该函数调用时一直添加x,y坐标
        y_data.append(event.ydata)
        plt.plot(x_data,y_data,c='m')
        plt.draw()
    if event.name == 'motion_notify_event' and pressed==1 and event.inaxes is not None and cir+lin!=0:
        ...
        if cir==1 and lin==0:
            r = ax.text((x1 + x2) / 2 + 0.5, (y1 + y2) / 2 + 0.5, 'r={}'.format(round(ra, 2)),
                        fontdict={'fontsize': 10, 'fontweight': 'light', 'color': 'blue'})
        if cir==0 and lin==1:
            r = ax.text((x1 + x2) / 2 + 0.5, (y1 + y2) / 2 + 0.5, 's={}'.format(round(ra, 2)),
                        fontdict={'fontsize': 10, 'fontweight': 'light', 'color': 'blue'})
        lines.append(line)
        radius.append(r)
        plt.draw()
    ...
    if fig is None or ax is None:
        print("Figure or Axes object is None")
    # if event.name == 'motion_notify_event' and pressed == 1 and event.inaxes is not None and cir == 0 and lin == 1:
...
def clear_all(event):
    ...
    if event.key=='delete':
        ...
        ax.set_ylim(-10, 10)
        ax.set_aspect('equal')
        fig.canvas.draw()    # 清除所有，重建坐标轴，但是cir不变（模式不变）
        lines = []
        radius = []
        exist_radius = []
        exist_lines = []
